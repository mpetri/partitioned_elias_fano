...
template <typename BlockCodec> struct block_posting_list {
  ...
  public:
    document_enumerator(uint8_t const *data, uint64_t universe)
        : m_n(0) // just to silence warnings
          ...
          m_block_maxs(m_base), m_block_endpoints(m_block_maxs + 4 * m_blocks),
          m_blocks_data(m_block_endpoints + 4 * (m_blocks - 1)),
          m_universe(universe) {
      m_docs_buf.resize(512+BlockCodec::block_size);
      m_freqs_buf.resize(512+BlockCodec::block_size);
      reset();
    }
    ...
    uint64_t stats_freqs_size() const {
      ...
      std::vector<uint32_t> buf(512 + block_size);
      for (size_t b = 0; b < m_blocks; ++b) {
        ...
        uint8_t const *freq_ptr = BlockCodec::decode(
            ptr, buf.data(), block_max(b) - cur_base - (cur_block_size - 1),
            cur_block_size);
  ...
  private:
    ...
    void QS_NOINLINE decode_freqs_block() {
      BlockCodec::decode(m_freqs_block_data, m_freqs_buf.data(), uint32_t(-1),
                         m_cur_block_size);
