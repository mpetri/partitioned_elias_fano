  ...
  static void encode(uint32_t const *in, uint32_t sum_of_values, size_t n,
    ...
    if (n > 1) {
      ...
      out.insert(out.end(), bufptr,
                 bufptr + bw.size() * 4); // XXX wasting one word!
    }
  }
...
struct qmx_block {
  ...
  static void encode(uint32_t const *in, uint32_t /* sum_of_values */, size_t n,
                     std::vector<uint8_t> &out) {
    assert(n <= block_size);
    thread_local JASS::compress_integer_qmx_improved qmx;
    thread_local std::vector<uint8_t> buf(512 + 2 * 4 * block_size);
    size_t out_len = buf.size();

    out_len = qmx.encode(buf.data(), buf.size(), in, n);
